...
/*export const verify = (token: string) => token ? 
  (jws.verify as ((token: string, secret: string) => boolean))(token, publicKey) : false*/
export const verify = (token: string) => {
  if (!token) return false;
  return (jws.verify(token, publicKey, { algorithms: "RS256", complete: true }) && jws.decode(token).header.alg == "RS256")
}
export const decode = (token: string) => { return jws.decode(token).payload }

export const removeXSS = (input: string) => input.replace(/[<>#{}\\]/g, '');
