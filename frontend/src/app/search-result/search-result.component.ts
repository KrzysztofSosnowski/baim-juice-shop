...
const removeXSS = (input: string) => input.replace(/[<>\\/]/g, '');
...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  trustProductDescription (tableData: any[]) { // vuln-code-snippet neutral-line restfulXssChallenge
    for (let i = 0; i < tableData.length; i++) { // vuln-code-snippet neutral-line restfulXssChallenge
      let tmp = removeXSS(tableData[i].description)
      tableData[i].description = this.sanitizer.bypassSecurityTrustHtml(tmp) // vuln-code-snippet vuln-line restfulXssChallenge
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      let tmp = removeXSS(queryParam)
      this.searchValue = this.sanitizer.bypassSecurityTrustHtml(tmp) // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge
